...
/*-----------------------------------------------*/
/* ZigZagzag-order to raster-order conversion table */
/*-----------------------------------------------*/

static const uint8_t ZigZag[] = {	/* ZigZagzag-order to raster-order conversion table */
...
)
{
	...
	while (ndata) {	/* Process all tables in the segment */
		...
		for (i = 0; i < 64; i++) {				/* Load the table */
			zi = ZigZag[i];						/* ZigZagzag-order to raster-order conversion */
...
)
{
	...
	for (blk = 0; blk < nby + 2; blk++) {	/* Get nby Y blocks and two C blocks */
		...
		if (cmp && jd->ncomp != 3) {		/* Clear C blocks if not exist (monochrome image) */
			...
			z = 1;		/* Top of the AC elements (in ZigZagzag-order) */
			do {
				...
				if (bc &= 0x0F) {					/* Bit length? */
					...
					i = ZigZag[z];						/* Get raster-order index */
